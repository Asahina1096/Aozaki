...
Aozaki is a modern ServerStatus-Rust frontend monitoring dashboard built with Astro, React, Panda CSS, and shadcn/ui. It displays real-time server monitoring data fetched from a ServerStatus-Rust backend API.

**Tech Stack:**
- **Astro 5.x**: Static site generator (Islands Architecture)
- **React 19**: Interactive UI components with React Compiler enabled
- **Panda CSS 1.x**: Zero-runtime CSS-in-JS styling framework
- **TypeScript 5.x**: Type system
- **shadcn/ui**: UI component library (migrated to Panda CSS)
...
3. Import Panda CSS utilities: `import { css, cx } from "../../styled-system/css"`
4. Import shadcn/ui components from `./ui/`
5. Use `@/*` imports for project files
6. In Astro files, add `client:load` directive when used:
   ```astro
...
// Inline styles
<div className={css({ bg: "primary", color: "white", p: "4" })} />
...
<div className={cx(baseStyle, css({ mt: "4" }), className)} />
```
...
css({
  ...
  _focus: { outline: "2px solid", outlineColor: "ring" },
  _disabled: { opacity: "0.5" }
})
```

**Design Tokens:**
- Colors: `primary`, `secondary`, `accent`, `muted`, `destructive`, `border`, `background`, `foreground`
- Spacing: Use numeric values like `p: "4"`, `mt: "8"`, `gap: "2"`
- Radii: `rounded: "sm"`, `rounded: "md"`, `rounded: "lg"`
- All tokens are defined in `panda.config.ts` and map to CSS variables
...
<header class={headerClass}>
  <div class={css({ maxW: "1400px", mx: "auto", px: "8" })}>
    <!-- content -->
  </div>
</header>
```
...
**Configuration File:** `panda.config.ts`
- Defines design tokens (colors, spacing, radii)
- Configures semantic tokens that reference CSS variables
- Sets up keyframes for animations
- Specifies file patterns to scan for styles

**Important Commands:**
```bash
bun panda codegen        # Generate styled-system (runs automatically on install)
```

**Key Features:**
- **Zero Runtime**: All styles are extracted at build time
- **Type Safety**: Full TypeScript support with autocomplete
- **CSS Variables**: All tokens map to CSS variables defined in `src/styles/panda.css`
- **Responsive**: Built-in responsive design with breakpoint modifiers
- **Dark Mode**: Automatic dark mode support via `.dark` class

**Do Not Edit:**
- The `styled-system/` directory is auto-generated
- Run `bun panda codegen` after modifying `panda.config.ts`
...
- Uses Panda CSS 1.x for zero-runtime styling
- Custom color scheme defined via CSS variables in `src/styles/panda.css`
- Design tokens configured in `panda.config.ts`
...
- All styles are type-safe with full TypeScript autocomplete
...
2. **Styling Framework**: Uses Panda CSS (not TailwindCSS) - import `css` and `cx` from `styled-system/css`
3. **Linting/Formatting**: Uses Biome, not ESLint/Prettier - run `bun run biome:fix` to auto-fix issues
4. **React Version**: React 19 is used; ensure compatibility when adding new libraries
5. **Path Aliases**: Use `@/*` instead of relative imports (e.g., `@/lib/api` not `../../lib/api`)
6. **Client Directives**: Never forget `client:*` on React components in `.astro` files
7. **API Dependency**: The application requires a running ServerStatus-Rust backend exposing `/json/stats.json`
8. **Islands Architecture**: Prefer Astro components for static content (see Header.astro and Footer.astro as examples), use React with `client:*` directives only for interactive components
9. **Panda CSS Codegen**: The `styled-system/` directory is auto-generated - run `bun panda codegen` after modifying `panda.config.ts`
10. **CSS Variables**: All design tokens map to CSS variables defined in `src/styles/panda.css` for theme consistency
