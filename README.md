# Komari AstroNext 主题

一个现代化的 Komari 监控系统主题，基于 Astro、React、TailwindCSS 和 shadcn/ui 构建。

## 特性

- ✨ 现代化的暗色响应式设计
- 🎨 基于 TailwindCSS 和 shadcn/ui 的精美 UI 组件
- 🚀 Astro 静态站点生成，性能出色
- ⚡ React 客户端组件实现动态交互
- 🔄 实时数据刷新（通过 JSON-RPC2）
- 📱 完全响应式布局，支持移动端
- 🌙 明暗主题切换
- ⚙️ 可配置的主题设置（显示模式、刷新间隔等）

## 技术栈

- **框架**: Astro 4.x
- **UI 库**: React 18
- **样式**: TailwindCSS + shadcn/ui
- **图标**: Lucide React
- **类型**: TypeScript
- **数据获取**: JSON-RPC2

## 快速开始

### 安装依赖

```bash
bun install
```

### 开发

```bash
bun run dev
```

访问 `http://localhost:4321` 查看主题。

### 构建

```bash
bun run build
```

构建输出将生成在 `dist/` 目录中。

### 预览

```bash
bun run preview
```

## 部署到 Komari

### 1. 构建主题

```bash
bun run build
```

### 2. 打包主题

将以下文件打包成 ZIP 文件：

```
komari-astronext.zip
├── komari-theme.json
├── dist/
│   ├── index.html
│   ├── _astro/
│   └── favicon.svg
```

使用以下命令打包：

```bash
zip -r komari-astronext.zip komari-theme.json dist/
```

### 3. 上传主题

1. 登录 Komari 管理后台
2. 进入主题管理页面
3. 上传 `komari-astronext.zip`
4. 激活主题

## 主题配置

主题支持以下配置项（可在 Komari 管理后台设置）：

- **默认视图模式**: 网格 / 列表
- **显示离线节点**: 是否显示离线的节点
- **刷新间隔**: 自动刷新节点状态的时间间隔（秒）
- **自动刷新**: 是否自动刷新节点数据
- **紧凑模式**: 启用紧凑模式以在有限空间内显示更多节点

## RPC2 接口

主题通过 `/api/rpc2` 调用以下 Komari 接口：

- `common:getNodes` - 获取所有节点信息
- `common:getNodesLatestStatus` - 获取节点最新状态
- `common:getPublicInfo` - 获取公开站点信息

## 项目结构

```
src/
├── components/         # React 组件
│   ├── ui/            # shadcn/ui 基础组件
│   ├── Header.tsx     # 页头组件
│   ├── Footer.tsx     # 页脚组件
│   ├── NodeCard.tsx   # 节点卡片组件
│   └── NodesGrid.tsx  # 节点网格组件
├── layouts/           # Astro 布局
│   └── BaseLayout.astro
├── lib/               # 工具库
│   ├── rpc2.ts       # RPC2 客户端
│   ├── utils.ts      # 工具函数
│   └── types/        # TypeScript 类型定义
├── pages/            # Astro 页面
│   └── index.astro   # 主页
└── styles/           # 全局样式
    └── globals.css
```

## 兼容性

- Komari 服务器版本: >= 1.0.7
- 浏览器: 现代浏览器（Chrome, Firefox, Safari, Edge）

## 自定义开发

### 添加新组件

在 `src/components/` 目录中创建新的 React 组件。

### 修改样式

编辑 `src/styles/globals.css` 或在组件中使用 TailwindCSS 类。

### 添加新页面

在 `src/pages/` 目录中创建新的 `.astro` 文件。

## 本地存储

主题使用以下本地存储字段：

- `appearance`: 明暗主题设置 (light / dark / system)
- `nodeSelectedGroup`: 用户选择的节点分组
- `nodeViewMode`: 展示模式 (grid / table)

## 许可证

MIT License

## 作者

Komari Dev Team

## 文档

- **[Claude.md](./Claude.md)** - 完整的开发和部署文档（包含架构、开发指南、部署指南）
- **[rpc.md](./rpc.md)** - RPC2 接口参考文档
- **[theme.md](./theme.md)** - Komari 主题开发指南

## 相关链接

- [Komari 监控系统](https://github.com/komari-monitor/komari)
- [Astro 文档](https://docs.astro.build/)
- [shadcn/ui 文档](https://ui.shadcn.com/)
