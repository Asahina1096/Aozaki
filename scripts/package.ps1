# Komari AstroNext 主题打包脚本 (PowerShell)

Write-Host "🚀 开始构建 Komari AstroNext 主题..." -ForegroundColor Green
...
if (Test-Path "komari-astronext.zip") {
    Remove-Item -Force "komari-astronext.zip"
}
...
Compress-Archive -Path ".package-temp/*" -DestinationPath "komari-astronext.zip" -Force

# 清理临时文件
Remove-Item -Recurse -Force ".package-temp"

Write-Host "✅ 打包完成！" -ForegroundColor Green
Write-Host "📦 主题包: komari-astronext.zip" -ForegroundColor Cyan
Write-Host ""
Write-Host "部署步骤：" -ForegroundColor Yellow
Write-Host "1. 登录 Komari 管理后台"
Write-Host "2. 进入主题管理页面"
Write-Host "3. 上传 komari-astronext.zip"
Write-Host "4. 激活主题"
